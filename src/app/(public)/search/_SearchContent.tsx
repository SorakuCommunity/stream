  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <select value={sort} onChange={(e) => setSort(e.target.value)} className="soraku-input w-auto text-xs py-1.5">
          {SORTS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
        <select value={genre} onChange={(e) => setGenre(e.target.value)} className="soraku-input w-auto text-xs py-1.5">
