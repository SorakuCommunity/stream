...
import { Play, Download, Share2, Check, ChevronDown, ChevronUp } from "lucide-react";
...
export function WatchClient({ anime }: WatchClientProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${consumetBase}/anime/gogoanime/${encodeURIComponent(slug)}`)
      .then((r) => r.json())
      .then((data) => {
        const eps: EpisodeInfo[] =
          data?.results?.[0]?.episodes?.map((e: { id: string; number: number; title?: string }) => ({
            id: e.id,
            number: e.number,
            title: e.title,
          })) ?? [];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      {/* Controls row */}
      <div
        className="flex flex-wrap items-center gap-2 p-3 rounded-lg mb-4"
        style={{ backgroundColor: "var(--bg-card)" }}
      >
        <span className="text-sm flex-1" style={{ color: "var(--text-secondary)" }}>
          {currentEp
            ? <>Menonton <strong style={{ color: "var(--text-primary)" }}>Episode {currentEp.number}</strong></>
            : <span className="skeleton inline-block w-32 h-4 rounded" />}
        </span>

        {/* Source/Language toggles */}
        <div className="flex gap-1.5 flex-wrap">
          {[
            { s: "default", l: "sub", label: "Sub — Default" },
            { s: "vidstreaming", l: "sub", label: "Sub — VS" },
            { s: "default", l: "dub", label: "Dub" },
          ].map(({ s, l, label }) => (
            <button
              key={`${s}-${l}`}
              onClick={() => { setSourceType(s); setLanguage(l); }}
              className={cn("soraku-btn text-xs py-1 px-2.5",
        ...
        <div className="flex gap-1.5">
          ...
          <button onClick={handleShare} className="soraku-btn p-2 gap-1.5 text-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div>
          ...
          {anime.description && (
            <div>
              <p
                className={cn("text-sm leading-relaxed transition-all", !showDesc && "line-clamp-3")}
            ...
            {loadingEps
              ...
              : episodes.map((ep) => (
                  ...
                  >
                    Episode {ep.number}
                    {ep.title && (
                      <span className="ml-1 opacity-70 line-clamp-1"> — {ep.title}</span>
                    )}
                  </button>
                ))}
          </div>
